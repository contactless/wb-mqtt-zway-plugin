 ...
 */
WBMQTTClient.buildFixedHeader = function (message_type, dup_flag, qos_level, retain, remaining_length) {
  var la = [];
...
WBMQTTClient.buildMessage = function () {
  var length = 0;
...
WBMQTTClient.decodeHeader = function (fixed_header) {
  var ret = {};
 ...
 */
WBMQTTClient.prototype.onError = function (callback) { this._errorCallback = callback; };
WBMQTTClient.prototype.onLog = function (callback) { this._logCallback = callback; };
WBMQTTClient.prototype.onDisconnect = function (callback) { this._disconnectCallback = callback; };
WBMQTTClient.prototype.onConnect = function (callback) { this._connectionCallback = callback; };

/**
 * Public interface
 */
WBMQTTClient.prototype.connect = function () {
  var self = this;
...
WBMQTTClient.prototype.close = function () {
  var self = this;
...
WBMQTTClient.prototype.publish = function (topic, payload, options, callback) {
  if (!this.connected) {
  ...
  var topic_name = WBMQTTClient.buildMessage(topic_length, topic);
  ...
  // Fixed header
  var fixed_header = WBMQTTClient.buildFixedHeader(WBMQTTClient.messageTypes.PUBLISH,
      options.dup_flag, options.qos_level, options.retain,
      topic_name.length + message_id.length + payload.length);

  var buffer = WBMQTTClient.buildMessage(fixed_header, topic_name, message_id, payload);
  this._send(buffer);
...
WBMQTTClient.prototype.subscribe = function (topic, options, callback) {
  if (!this.connected) {
    this._onError(Error('Please connect to server first'));
    return;
  }
  ...
  var payload = WBMQTTClient.buildMessage(topic_length, topic, requested_qos);

  // Fixed Header
  var fixed_header = WBMQTTClient.buildFixedHeader(WBMQTTClient.messageTypes.SUBSCRIBE, options.dup_flag, 1, 0,
      message_id.length + payload.length);

  var buffer = WBMQTTClient.buildMessage(fixed_header, message_id, payload);
  this._send(buffer);
...
WBMQTTClient.prototype._onTimeout = function () {
  this._onError(Error('Timeout'));
  this._connection.close();
};

WBMQTTClient.prototype._onData = function (chunk) {
  if (Buffer.isBuffer(this._fixed_header_chunk)) {
    this._onData(WBMQTTClient.buildMessage(this._fixed_header_chunk, chunk));
  } else {
    var fixed_header = WBMQTTClient.decodeHeader(chunk);
    if (fixed_header == false) {
      ...
      var handler = WBMQTTClient.messageHandlers[fixed_header.message_type];
...
WBMQTTClient.prototype._onPublish = function (topic, payload) {
 ...
 */
WBMQTTClient.prototype._clearPingTimer = function () {
  if (this._ping_timer) {
...
WBMQTTClient.prototype._clearPingTimeoutTimer = function () {
...
WBMQTTClient.prototype._clearConnectTimer = function () {
...
WBMQTTClient.prototype._startSession = function () {
  var self = this;
  ...
  // Payload
  var payload = WBMQTTClient.buildMessage(client_id_length, client_id,
  ...
  // Fixed Header
  var fixed_header = WBMQTTClient.buildFixedHeader(WBMQTTClient.messageTypes.CONNECT, 0, 0, false, variable_header.length + payload.length);

  if (this.options.infoLogEnabled === true)
    this._onLog("CNCT FIXED HEAD: " + fixed_header.toString('hex'));

  var buffer = WBMQTTClient.buildMessage(fixed_header, variable_header, payload);
...
WBMQTTClient.prototype._send = function (buffer) {
  if (this.options.infoLogEnabled === true)
...
WBMQTTClient.prototype._response = function (qos_level, message_id) {
  var buffer = new Buffer(4);
  if (qos_level == 1)
    buffer[0] = WBMQTTClient.messageTypes.PUBACK << 4;
  else
    buffer[0] = WBMQTTClient.messageTypes.PUBREC << 4;
...
WBMQTTClient.prototype._ping = function () {
  var self = this;
  ...
  buffer[0] = WBMQTTClient.messageTypes.PINGREQ << 4;
  buffer[1] = 0x00;
 ...
 */
WBMQTTClient.messageHandlers = [];
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.CONNACK] = function (self, fixed_header, chunk) {
  // Clear timeout
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PUBACK] = function (self, fixed_header, chunk) {
  if (chunk.length < 4)
    self._onError(Error('CONNACK format error'));
  else {
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PUBREC] = function (self, fixed_header, chunk) {
  if (chunk.length < 4)
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PUBREL] = function (self, fixed_header, chunk) {
  // Ignore
};

WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PUBCOMP] = function (self, fixed_header, chunk) {
  if (chunk.length < 4)
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.SUBACK] = function (self, fixed_header, chunk) {
  if (chunk.length < 5)
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.UNSUBACK] = function (self, fixed_header, chunk) {
  if (chunk.length < 4)
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PINGRESP] = function (self, fixed_header, chunk) {
  if (chunk.length < 2) {
...
WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PUBLISH] = function (self, fixed_header, chunk) {
  if (self._data_not_enough) {
    if (self._data_offset + chunk.length >= self._data_length) {
      self._data_not_enough = false;
      WBMQTTClient.messageHandlers[WBMQTTClient.messageTypes.PUBLISH](self, fixed_header, WBMQTTClient.buildMessage(self._data_chunk, chunk));
    } else {
